...
// The DB stores a single img_dir, but every uploaded file is named with an
// `_AR<auditResultId>_` marker, so multiple photos for the same record are
// discoverable on disk without a schema change.
app.get('/api/audit/result/:auditResultId/images', async (req, res) => {
    ...
    try {
        // Primary image recorded on the audit_result (may be null).
        let primary = null;
        try {
            const [rows] = await execQuery('SELECT img_dir FROM audit_results WHERE id = ?', [auditResultId]);
        ...
        // Scan the uploads tree for files tagged with this audit_result id.
        let entries = [];
        try {
        ...
        const addFile = (relUnderUploads) => {
            const rel = String(relUnderUploads).split(path.sep).join('/');
            ...
            seen.add(rel);
            images.push({ path: `uploads/${rel}`, url: `/uploads/${rel}`, mtime });
        };
        ...
        // Ensure the DB-recorded primary is included even if it lacks the marker.
        if (primary) {
            addFile(primary.replace(/^uploads[\\/]/, ''));
        }

        images.sort((a, b) => a.mtime - b.mtime);
        const result = images.map(({ path, url }) => ({ path, url }));

        console.log(`   ⬅️ STATUS: 200 OK. ${result.length} image(s) for audit_result ${auditResultId}`);
        return res.status(200).json({
