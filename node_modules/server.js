...
app.post('/api/assets', async (req, res) => {
    ...
    const allowed = ['Desktop','Laptop','Monitor','Printer','UPS','Computer'];
...
// 1c. DELETE: Remove an asset by ID
app.delete('/api/assets/:assetId', async (req, res) => {
    const { assetId } = req.params;

    const timestamp = new Date().toISOString();
    const clientIp = req.headers['x-forwarded-for'] || (req.socket && req.socket.remoteAddress) || req.ip || 'unknown';
    console.log(`\n[${timestamp}] DELETE /api/assets/${assetId} from ${clientIp}`);

    // Validate asset ID
    if (!assetId || isNaN(assetId)) {
        console.warn('   ⚠️ Invalid asset ID');
        return res.status(400).json({ message: 'Invalid asset ID provided' });
    }

    try {
        // First check if asset exists
        const [existingAsset] = await execQuery('SELECT id, asset_tag FROM assets WHERE id = ?', [assetId]);
        
        if (existingAsset.length === 0) {
            console.warn(`   ⚠️ Asset ID ${assetId} not found`);
            return res.status(404).json({ message: 'Asset not found', asset_id: assetId });
        }

        const assetTag = existingAsset[0].asset_tag;
        console.log(`   ✅ Asset found: ID ${assetId}, Tag: ${assetTag}`);
        ...
        if (resultCount > 0) {
            console.warn(`   ⚠️ Asset ${assetId} has ${resultCount} related audit result(s). Cannot delete due to foreign key constraint.`);
        ...
        console.log(`   ⬅️ STATUS: 200 OK. Asset deleted. ID: ${assetId}, Tag: ${assetTag}`);
        return res.status(200).json({
