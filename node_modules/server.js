...
app.post('/api/audit/submit', async (req, res) => {
    ...
    try {
        if (asset_id != null) {
            ...
            const [rows] = await pool.execute('SELECT id FROM assets WHERE glpi_id = ?', [asset_id]);
            if (rows.length > 0) {
                ...
                const [checkId] = await pool.execute('SELECT id FROM assets WHERE id = ?', [asset_id]);
                if (checkId.length === 0) {
    ...
    try {
        ...
        const [auditCheck] = await pool.execute('SELECT id FROM audits WHERE id = ?', [audit_id]);
        ...
        const [assetCheck] = await pool.execute('SELECT id FROM assets WHERE id = ?', [asset_id]);
...
app.post('/api/assets', async (req, res) => {
    ...
    try {
        ...
        const [existingRows] = await pool.execute(checkQuery, checkParams);
        ...
        if (existingRows.length > 0) {
            ...
            const [glpiRows] = await pool.execute(checkQuery, checkParams);
        ...
        if (existingAsset) {
            ...
            const [result] = await pool.execute(insertQuery, sanitizedValues);
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        // Create folder structure: uploads/YYYY/MM/DD
        const now = new Date();
        const year = now.getFullYear();
        const month = String(now.getMonth() + 1).padStart(2, '0');
        const day = String(now.getDate()).padStart(2, '0');
        const folderPath = path.join(uploadDir, year.toString(), month, day);
    ...
    filename: (req, file, cb) => {
        // Filename format: COMPUTERNAME_DDMMYY.ext
        const now = new Date();
        const dd = String(now.getDate()).padStart(2, '0');
        const mm = String(now.getMonth() + 1).padStart(2, '0');
        const yy = String(now.getFullYear()).slice(-2);
        const ext = path.extname(file.originalname);
        const computerName = req.body.computer_name || 'UNKNOWN';
        const filename = `${computerName}_${dd}${mm}${yy}${ext}`;
        cb(null, filename);
    }
});
...
app.post('/api/audit/upload-image', upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        const { audit_result_id, computer_name } = req.body;

        // Validate required fields
        if (!audit_result_id || !computer_name) {
            console.warn('   ⚠️ Missing required fields: audit_result_id or computer_name');
            return res.status(400).json({ message: 'Missing required fields: audit_result_id, computer_name' });
        }
        ...
        const month = String(now.getMonth() + 1).padStart(2, '0');
        const day = String(now.getDate()).padStart(2, '0');
        const imgPath = `uploads/${year}${month}${day}/${req.file.filename}`;

        console.log(`   📁 Image saved to: ${req.file.path}`);
        console.log(`   📸 Filename: ${req.file.filename}`);
        console.log(`   💾 DB path: ${imgPath}`);

        // Update audit_results table with image path
        const updateQuery = `
            UPDATE audit_results
            SET img_dir = ?
            WHERE id = ?
        ...
        const [result] = await pool.execute(updateQuery, [imgPath, audit_result_id]);
