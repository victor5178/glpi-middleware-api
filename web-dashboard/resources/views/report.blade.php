    ...
    @if ($ran)
        ...
        @php
            ...
            $fmt = fn ($t) => ! empty($t)
                ...
                : '—';
            // All photo URLs for a scanned item (multi-photo aware, deduped).
            $imgs = function ($it) {
                $paths = [];
                if (! empty($it['images'])) {
                    foreach (explode('||', $it['images']) as $p) {
                        $p = trim($p);
                        if ($p !== '') $paths[] = $p;
                    }
                }
                if (empty($paths) && ! empty($it['img_dir'])) {
                    $paths[] = $it['img_dir'];
                }
                $seen = [];
                $urls = [];
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        @foreach ($items as $i => $it)
                            <tr>
                                <td>{{ $i + 1 }}</td>
                                @if($withPhotos)<td class="report-photo-cell">@php $urls = $imgs($it); @endphp @if(!empty($urls))<div class="report-photo-grid">@foreach($urls as $u)<img class="report-thumb" src="{{ $u }}" alt="" loading="lazy">@endforeach</div>@else<span class="muted">—</span>@endif</td>@endif
                                <td>{{ $it['asset_tag'] ?? '—' }}</td>
